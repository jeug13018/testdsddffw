<template>
  <div class="p-4 max-w-3xl mx-auto text-xs">
        ...
        <table class="w-full border-collapse">
          <tbody>
            <tr class="border-b border-gray-200">
              ...
              <td class="py-2 pl-4" v-if="post.city || post.province">
                {{ post.province }} <span v-if="post.city"> · {{ post.city }}</span>
              </td>
    ...
    <ImageViewer v-model:visible="imgDialog" :images="post.media" :blurAmount="imageViewerBlur" :initialIndex="initialIndex" />
    <!-- 新手指南 -->
...
// ImageViewer的模糊程度使用计算属性
const imageViewerBlur = computed(() => {
  if (post.value.type === '站街') {
    if (post.value.isVip) {
      return 0
    }
    return 3
...
// BlurredImage的模糊程度使用计算属性
const blurredImageBlur = computed(() => {
  if (post.value.type === '站街') {
    if (post.value.isVip) {
      return 0
    }
