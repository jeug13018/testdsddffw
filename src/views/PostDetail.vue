<template>
  <div class="p-4 max-w-3xl mx-auto text-xs">
    ...
    <template v-else>
      ...
      <div v-if="post.content" class="mb-4 p-4 bg-white border-l-4 border-l-pink-400 border-t border-r border-b border-gray-200 rounded-md text-sm leading-6 shadow-md">{{ post.type === '站街' ? maskText(post.content) : post.content }}</div>
    ...
    <ImageViewer v-model:visible="imgDialog" :images="post.media" :initialIndex="initialIndex" />
    <!-- 新手指南 -->
