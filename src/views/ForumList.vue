<template>
  <div class="p-4 max-w-3xl mx-auto z-10">
    <div class="mb-2 bg-white rounded-lg shadow-sm border p-4">
      ...
      <div class="space-y-1">
        ...
        <div class="flex items-start gap-3 p-1 rounded hover:bg-neutral-50 cursor-pointer" @click="joinTGChannel">
          <div class="w-2 h-2 bg-red-500 rounded-full mt-2 flex-shrink-0"></div>
          <div class="flex-1 min-w-0 flex items-center justify-between">
            <div class="text-sm font-medium text-neutral-800 mb-1">加入tg频道【信息更及时准确】</div>
    ...
    <div class="mb-4">
      ...
      <div class="flex gap-2 flex-wrap mb-2">
        <button
          class="px-3 py-1 rounded text-xs bg-neutral-800 text-white"
        >
          {{ selectedCity || '全部' }}
        </button>
        <button
      ...
      <div v-if="showAllCities" class="border rounded p-2 bg-white shadow">
        <div v-for="(cities, province) in provinceCityMap" :key="province" class="mb-3">
          ...
          >
            <span class="mr-2">{{ expandedProvinces.includes(province) ? '▼' : '▶' }}</span>
            {{ province }}
          </div>
    ...
    <div class="grid gap-4" v-else-if="posts.length > 0">
      <div
        v-for="post in posts"
        :key="post.id"
        class="flex items-center justify-between bg-white rounded shadow-sm py-4 px-3 transition-all hover:bg-accent bg-muted cursor-pointer"
      ...
      >
        <div class="flex flex-col justify-between h-full" style="flex: 1;">
          ...
          <div class="flex items-center gap-2">
            <span class="px-2 py-0.5 rounded bg-blue-100 text-blue-700 text-xs">{{ post.city }} · {{ post.area }}</span>
            <span class="px-2 py-0.5 rounded bg-gray-100 text-gray-500 text-xs truncate">{{ formatSmartTime(post.create_time) }}</span>
          </div>
          <div class="text-neutral-500 text-xs max-w-full line-clamp-2" style="display:-webkit-box;-webkit-line-clamp:2;-webkit-box-orient:vertical;overflow:hidden;">{{ post.type === '站街' ? maskText(post.content) : post.content }}</div>
        </div>
...
async function fetchPosts() {
  ...
  try {
    ...
    if (selectedCity.value && selectedCity.value !== '全部') {
...
function maskText(text) {
  ...
  for (let i = 0; i < text.length; i += 4) {
    ...
    // 下两个字符替换为星号(如果存在)
    if (i + 2 < text.length) {
      const remainingChars = Math.min(2, text.length - (i + 2));
      result += '*'.repeat(remainingChars);
    }
  }
  return result;
}
